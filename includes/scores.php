...
function refreshScores($url, $game_settings = null){
	...
	if($home_team['playingPeriodScores'][4]){
		$games_array['score'][4]['home'] =	($home_team['playingPeriodScores'][4]['score'] == "-" ? "-" : $games_array['score'][4]['home'] + intval($home_team['playingPeriodScores'][4]['score']));
	}
	//$games_array['score'][4]['home'] =	($home_team['playingPeriodScores'][3]['score'] == "-" ? "-" : $games_array['score'][3]['home'] + intval($home_team['playingPeriodScores'][3]['score']));
	...
	$games_array['score'][3]['away'] =	($away_team['playingPeriodScores'][2]['score'] == "-" ? "-" : $games_array['score'][2]['away'] + intval($away_team['playingPeriodScores'][2]['score']));
	$games_array['score'][4]['away'] =	($away_team['playingPeriodScores'][3]['score'] == "-" ? "-" : $games_array['score'][3]['away'] + intval($away_team['playingPeriodScores'][3]['score']));
	if($away_team['playingPeriodScores'][4]){
		$games_array['score'][4]['away'] =	($away_team['playingPeriodScores'][4]['score'] == "-" ? "-" : $games_array['score'][4]['away'] + intval($away_team['playingPeriodScores'][4]['score']));
	}
...
while($q > 0){
	...
	$url = "https://www.msn.com/en-us/sports/liteajax/nfl/FootballLeagueSchedule_nfl_FTM400_superbowl_ajax_mm";
		...
		//REAL SCORE
		$url = "https://api.msn.com/sports/livegames?apikey=kO1dI4ptCTTylLkPL1ZTHYP8JhLKb8mRDoA5yotmNJ&version=1.0&cm=en-us&activityId=6977c554-663a-48cb-ac4f-eed1df66b61f&it=web&user=m-2D3641899DAA6479163257649C38655E&scn=ANON&ids=7391a8d06675430592fcdc65bb4dcd38&scope=Full";

		//TEST
// 		$url = "https://api.msn.com/sports/livegames?apikey=kO1dI4ptCTTylLkPL1ZTHYP8JhLKb8mRDoA5yotmNJ&version=1.0&cm=en-us&user=m-20DC418084A963F00D64576B85AD62D9&lat=47.7712&long=-122.3828&activityId=697543d2-0a77-4b92-a633-499dc0aaf7e2&it=web&scn=ANON&ids=5848514c39774aa39db094ed5d0ebb34&scope=Full";
	...
	$force = true;
	if($seconds_since  > 20 || !$widget_array['created'] || $force){
		//RE POLL
		...
		$games_array = ($widget_array['data']);
	}
	
	$history = json_decode(file_get_contents("history.json"), true);
	$games_array['history'] = $history;
	$games_array = json_encode($games_array);
	echo $games_array;
	exit();
